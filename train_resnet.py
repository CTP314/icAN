...
class ICAN:
    ...
    def train(self, batch):
        ...
        icon_src_enc = self.generator.encoder(icon_src)
        ...
        icon_fake_enc = self.generator.encoder(icon_fake)
    ...
    def generate(self, icon_src, theme_id):
        ...
        with torch.no_grad():
            ...
            icon_tar = self.generator(icon_src, theme_id).squeeze().permute(1, 2, 0).cpu().numpy()
        return icon_tar
                ...
                for theme in train_dataset.themes:
                    if theme in train_dataset.label2theme[label]:
                        ...
                        for theme_tar in train_dataset.themes:
                            icon_tar = trainer.generate(icons[0], theme_id=train_dataset.theme2id[theme_tar])
                            icons.append(icon_tar)
                    else:
                        icons = [np.zeros((128, 128, 4))]
                        for theme_tar in train_dataset.themes:
                            icons.append(np.zeros((128, 128, 4)))     
                    icon_output.append(np.concatenate(icons, axis=1))                        

                icon_output = ((np.concatenate(icon_output, axis=0) + 1) / 2 * 255).astype(np.uint8)
